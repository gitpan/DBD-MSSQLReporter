package DBD::MSSQLReporter;

# Name:
#	DBD/MSSQLReporter.pm.
#
# Purpose:
...
# can use when calling DBD::MSSQLReporter -> new().
...
DBD::MSSQLReporter - An module to connect Perl to MS SQL Server and MS Data Engine
...
you should not even have to worry about the DSN.
	...
	#!perl -w
	use strict;
	use DBD::MSSQLReporter;
	
	my($connect) = "dbi:ODBC(RaiseError=>1, PrintError=>1, Taint=>1):DSN=LocalServer";
	my($reporter) = DBD::MSSQLReporter -> new(connexion => $connect);
...
C<DBD::MSSQLReporter> encapsulates the connection between Perl and MS SQL Server.

C<DBD::MSSQLReporter> was written so that I could teach myself about MS SQL Server and
...
You install C<DBD::MSSQLReporter>, as you would install any perl module,
...
C<DBD::MSSQLReporter> was written by Ron Savage I<E<lt>ron@savage.net.auE<gt>> in 2000.
